...
class AdminContactController:
    def __init__(self, api):
        self.api = api
        self.admin_contact_model = api.model(
            "AdminContact",
            {
                "admin_id": fields.String(required=True, description="Admin Id"),
                "mobile_isd_call": fields.String(
                    required=True, description="Admin Mobile ISD"
                ),
                "mobile_no_call": fields.String(
                    required=True, description="Admin Mobile No"
                ),
                "mobile_isd_watsapp": fields.String(
                    required=True, description="Admin Whatsapp ISD"
                ),
                "mobile_no_watsapp": fields.String(
                    required=True, description="Admin Whatsapp No"
                ),
        ...
        self.admin_contact_ns = Namespace(
            "admin_contact",
            ...
            def get(self):
                try:
                    ...
                    if not admin_contactes_data:
                        return jsonify(
                            {"message": "No Admin Contact found", "status": 404}
                        )
                    else:
                        return jsonify(
                            {
                                "message": "Admin Contact found Successfully",
                                "status": 200,
                                "data": admin_contactes_data,
                            }
                        )
            ...
            def post(self):
                try:
                    ...
                    if not mobile_isd_call:
                        return jsonify(
                            {"message": "Please Provide Mobile ISD", "status": 201}
                        )
                    if not mobile_no_call:
                        return jsonify(
                            {"message": "Please Provide Mobile No", "status": 201}
                        )
                    if not mobile_isd_watsapp:
                        return jsonify(
                            {
                                "message": "Please Provide Whatsapp mobile ISD",
                                "status": 201,
                            }
                        )
                    if not email_id:
                        return jsonify(
                            {"message": "Please Provide Email Id", "status": 201}
                        )
                    else:
                        admin_contact = AdminContact(
                            admin_id=admin_id,
                            mobile_isd_call=mobile_isd_call,
                            mobile_no_call=mobile_no_call,
                            mobile_isd_watsapp=mobile_isd_watsapp,
                            mobile_no_watsapp=mobile_no_watsapp,
                            email_id=email_id,
                            is_active=1,
            ...
            def put(self, id):
                try:
                    ...
                    if not mobile_isd_watsapp:
                        ...
                        admin_contact = AdminContact.query.filter_by(
                            admin_id=id
                        ).first()
                        if not admin_contact:
                            return jsonify(
                                {"message": "Admin Contact not found", "status": 404}
                            )
                        else:
            ...
            def put(self, id):
                try:
                    ...
                    if not admin_contact:
                        ...
                        return jsonify(
                            {
                                "message": "Admin Contact activated successfully",
                                "status": 200,
                            }
                        )
                except Exception as e:
            ...
            def put(self, id):
                try:
                    ...
                    if not admin_contact:
                        ...
                        return jsonify(
                            {
                                "message": "Admin Contact deactivated successfully",
                                "status": 200,
                            }
                        )
                except Exception as e:
