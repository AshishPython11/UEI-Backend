...
from app.models.student import Contact, StudentAddress, Student, AcademicHistory,StudentLogin  # Adjust the import based on your model location
            ...
            def get(self):
                try:
                    ...
                    if google_user_info:
                        ...
                        # Fetch the student login entry using the Google email
                        student_login_entry = StudentLogin.query.filter_by(userid=google_email).first()
                        if not student_login_entry:
                            return jsonify({'message': 'Student login not found', 'status': 404})

                        student_login_id = student_login_entry.student_id
                    else:
                        # Use the current user ID directly as the login ID for normal users
                        student_login_id = current_user_id

                    # Query the student profile using the determined student_login_id
                    student = Student.query.filter_by(student_login_id=student_login_id, is_deleted=False).first()
                    if not student:
                        return jsonify({'message': 'Student not found', 'status': 404})

                    # Check for completion of basic info
                    ...
                    contact = Contact.query.filter_by(student_id=student_login_id, is_active=1).first()
                    ...
                    address = StudentAddress.query.filter_by(student_id=student_login_id, is_active=1).all()
                    ...
                    academic_history = AcademicHistory.query.filter_by(student_id=student_login_id, is_active=1).all()
